...
static void bench_keltnerchannel(void *ctx_) {
    ...
    const double *inputs[KELTNERCHANNEL_INPUTS] = {ctx->stock->high, ctx->stock->low, ctx->stock->close};
    double opts[KELTNERCHANNEL_OPTIONS] = {ctx->period, ctx->step};
...
typedef struct {
    ...
    const double (*option_sets)[KELTNERCHANNEL_OPTIONS]; // by_options: 4 option sets
} KeltnerChannelSimdCtx;

static void bench_keltnerchannel_simd_assets(void *ctx_) {
    KeltnerChannelSimdCtx *ctx = ctx_;
    // Each asset has KELTNERCHANNEL_INPUTS=3 input pointers (high, low, close)
    const double *inputs_per_asset[4][KELTNERCHANNEL_INPUTS] = {
    ...
    struct CSimdResult r = keltnerchannel_simd_by_assets((const double *const *const *)inputs, 4, ctx->data_len, ctx->opts, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] keltnerchannel_simd_by_assets failed: %d\n", (int) r.error);
        exit(1);
    }
...
static void bench_keltnerchannel_simd_options(void *ctx_) {
    KeltnerChannelSimdCtx *ctx = ctx_;
    const double *inputs[KELTNERCHANNEL_INPUTS] = {ctx->stocks->high, ctx->stocks->low, ctx->stocks->close};
    const double *opts[4] = {ctx->option_sets[0], ctx->option_sets[1],
                             ctx->option_sets[2], ctx->option_sets[3]};
    struct CSimdResult r = keltnerchannel_simd_by_options(inputs, ctx->data_len, opts, 4, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] keltnerchannel_simd_by_options failed: %d\n", (int) r.error);
        exit(1);
    }
    for (uintptr_t i = 0; i < r.num_results; i++) keltnerchannel_state_free(r.states[i]);
    tulip_ffi_simd_result_free(r);
...
static void run_keltnerchannel(const Stock *stocks, int num_stocks, int number, int repeat, int warmup) {
    static const double option_sets[][KELTNERCHANNEL_OPTIONS] = {{20.0, 2.0}, {20.0, 1.5}, {14.0, 2.0}, {10.0, 1.5}};
    printf("\n--- KELTNERCHANNEL ---\n");
    ...
    if (num_stocks >= 4) {
        ...
        KeltnerChannelSimdCtx sctx = { .stocks = stocks, .data_len = dlen };
        for (int o = 0; o < 4; o++) {
            sctx.opts = option_sets[o];
            TimingResult t_sa = time_fn(bench_keltnerchannel_simd_assets, &sctx, number, repeat, warmup);
            log_and_print("keltnerchannel", "tulip_rs_ffi_c_simd_by_assets", "All", option_sets[o], KELTNERCHANNEL_OPTIONS, t_sa, (int) dlen);
        }

        for (int s = 0; s < num_stocks; s++) {
            KeltnerChannelSimdCtx octx = { .stocks = &stocks[s], .data_len = stocks[s].len, .option_sets = option_sets };
            // One call times 4 option sets; first set logged as the representative key.
            TimingResult t_so = time_fn(bench_keltnerchannel_simd_options, &octx, number, repeat, warmup);
            log_and_print("keltnerchannel", "tulip_rs_ffi_c_simd_by_options", stocks[s].symbol, option_sets[0], KELTNERCHANNEL_OPTIONS, t_so, (int) stocks[s].len);
        }
    }
}
