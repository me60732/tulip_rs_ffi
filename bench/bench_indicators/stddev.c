...
typedef struct {
    ...
    const double (*option_sets)[STDDEV_OPTIONS]; // by_options: 4 option sets
} StdDevSimdCtx;
...
static void bench_stddev_simd_assets(void *ctx_) {
    StdDevSimdCtx *ctx = ctx_;
    // Each asset has STDDEV_INPUTS=1 input pointer (close price)
    const double *inputs_per_asset[4][STDDEV_INPUTS] = {
    ...
    struct CSimdResult r = stddev_simd_by_assets((const double *const *const *)inputs, 4, ctx->data_len, ctx->opts, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] stddev_simd_by_assets failed: %d\n", (int) r.error);
        exit(1);
    }
    for (uintptr_t i = 0; i < r.num_results; i++) stddev_state_free(r.states[i]);
    tulip_ffi_simd_result_free(r);
}

static void bench_stddev_simd_options(void *ctx_) {
    StdDevSimdCtx *ctx = ctx_;
    const double *inputs[STDDEV_INPUTS] = {ctx->stocks->close};
    const double *opts[4] = {ctx->option_sets[0], ctx->option_sets[1],
                             ctx->option_sets[2], ctx->option_sets[3]};
    struct CSimdResult r = stddev_simd_by_options(inputs, ctx->data_len, opts, 4, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] stddev_simd_by_options failed: %d\n", (int) r.error);
        exit(1);
    }
    for (uintptr_t i = 0; i < r.num_results; i++) stddev_state_free(r.states[i]);
    tulip_ffi_simd_result_free(r);
...
static void run_stddev(const Stock *stocks, int num_stocks, int number, int repeat, int warmup) {
    ...
    if (num_stocks >= 4) {
        ...
        StdDevSimdCtx sctx = { .stocks = stocks, .data_len = dlen };
        for (int o = 0; o < 4; o++) {
            sctx.opts = option_sets[o];
            TimingResult t_sa = time_fn(bench_stddev_simd_assets, &sctx, number, repeat, warmup);
            log_and_print("stddev", "tulip_rs_ffi_c_simd_by_assets", "All", option_sets[o], STDDEV_OPTIONS, t_sa, (int) dlen);
        }

        for (int s = 0; s < num_stocks; s++) {
            StdDevSimdCtx octx = { .stocks = &stocks[s], .data_len = stocks[s].len, .option_sets = option_sets };
            // One call times 4 option sets; first set logged as the representative key.
            TimingResult t_so = time_fn(bench_stddev_simd_options, &octx, number, repeat, warmup);
            log_and_print("stddev", "tulip_rs_ffi_c_simd_by_options", stocks[s].symbol, option_sets[0], STDDEV_OPTIONS, t_so, (int) stocks[s].len);
        }
    }
}
