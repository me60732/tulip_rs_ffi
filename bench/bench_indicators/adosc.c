...
static void bench_adosc(void *ctx_) {
    ...
    struct CIndicatorResult r = adosc_indicator(inputs, ctx->stock->len, opts, NULL, 0);
...
typedef struct {
    ...
    const double (*option_sets)[ADOSC_OPTIONS]; // by_options: 4 option sets
} AdoscSimdCtx;

static void bench_adosc_simd_assets(void *ctx_) {
    AdoscSimdCtx *ctx = ctx_;
    // Each asset has ADOSC_INPUTS=4 input pointers (high, low, close, volume)
    const double *inputs_per_asset[4][ADOSC_INPUTS] = {
    ...
    struct CSimdResult r = adosc_simd_by_assets((const double *const *const *)inputs, 4, ctx->data_len, ctx->opts, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] adosc_simd_by_assets failed: %d\n", (int) r.error);
        exit(1);
    }
...
static void bench_adosc_simd_options(void *ctx_) {
    AdoscSimdCtx *ctx = ctx_;
    const double *inputs[ADOSC_INPUTS] = {ctx->stocks->high, ctx->stocks->low, ctx->stocks->close, ctx->stocks->volume};
    const double *opts[4] = {ctx->option_sets[0], ctx->option_sets[1],
                             ctx->option_sets[2], ctx->option_sets[3]};
    struct CSimdResult r = adosc_simd_by_options(inputs, ctx->data_len, opts, 4, NULL, 0);
    if (r.error != C_INDICATOR_ERROR_OK) {
        fprintf(stderr, "[error] adosc_simd_by_options failed: %d\n", (int) r.error);
        exit(1);
    }
    for (uintptr_t i = 0; i < r.num_results; i++) adosc_state_free(r.states[i]);
    tulip_ffi_simd_result_free(r);
...
static void run_adosc(const Stock *stocks, int num_stocks, int number, int repeat, int warmup) {
    static const double option_sets[][ADOSC_OPTIONS] = {{2.0, 5.0}, {6.0, 20.0}, {5.0, 15.0}, {10.0, 30.0}};
    printf("\n--- ADOSC ---\n");
    ...
    if (num_stocks >= 4) {
        ...
        AdoscSimdCtx sctx = { .stocks = stocks, .data_len = dlen };
        for (int o = 0; o < 4; o++) {
            sctx.opts = option_sets[o];
            TimingResult t_sa = time_fn(bench_adosc_simd_assets, &sctx, number, repeat, warmup);
            log_and_print("adosc", "tulip_rs_ffi_c_simd_by_assets", "All", option_sets[o], ADOSC_OPTIONS, t_sa, (int) dlen);
        }

        for (int s = 0; s < num_stocks; s++) {
            AdoscSimdCtx octx = { .stocks = &stocks[s], .data_len = stocks[s].len, .option_sets = option_sets };
            // One call times 4 option sets; first set logged as the representative key.
            TimingResult t_so = time_fn(bench_adosc_simd_options, &octx, number, repeat, warmup);
            log_and_print("adosc", "tulip_rs_ffi_c_simd_by_options", stocks[s].symbol, option_sets[0], ADOSC_OPTIONS, t_so, (int) stocks[s].len);
        }
    }
}
