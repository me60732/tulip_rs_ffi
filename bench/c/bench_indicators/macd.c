...
static void bench_macd(void *ctx_) {
    ...
    double opts[3] = {ctx->short_period, ctx->long_period, ctx->signal_period};
...
static void bench_tulipc_macd(void *ctx_) {
    MacdCtx *ctx = ctx_;
    double options[3] = {ctx->short_period, ctx->long_period, ctx->signal_period};
...
static void run_macd(const Stock *stocks, int num_stocks, int number, int repeat, int warmup) {
    static const double option_sets[][3] = {
