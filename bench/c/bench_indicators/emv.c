...
static void bench_emv(void *ctx_) {
    EmvCtx *ctx = ctx_;
    const double *inputs[3] = {ctx->stock->high, ctx->stock->low, ctx->stock->volume};
    double opts[0] = {};
    struct CIndicatorResult r = emv_indicator(inputs, ctx->stock->len, opts, NULL, 0);
...
static void bench_tulipc_emv(void *ctx_) {
    ...
    double options[0] = {};
    int start_index = ti_emv_start(options);
    ...
    int ret = ti_emv((int) len, inputs, options, outputs);
