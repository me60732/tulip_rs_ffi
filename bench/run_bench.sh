...
# bench/.env -- see README.md.

# Re-execute under bash if launched via a POSIX shell (e.g. `sh run_bench.sh`):
# this script needs `set -o pipefail` and BASH_SOURCE, which dash doesn't have.
