...
int main(void) {
    ...
    printf("\n=== CMO: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[CMO_INPUTS] = {close};
        CIndicatorResult pr = cmo_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "cmo_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_CMO, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[CMO_INPUTS] = {close + PARTIAL};
        CBatchResult a = cmo_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = cmo_batch(rs, rinputs, REST, NULL, 0);
