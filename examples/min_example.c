// MIN (minimum) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_min_example.py: same sample data,
...
//
// MIN_INPUTS/MIN_OPTIONS come from ../include/tulip_rs_ffi_counts.h,
// generated by ../build.rs from the real tulip_rs::indicators::min::
...
//   cc -O2 -o min_example examples/min_example.c \
...
int main(void) {
    const double options[MIN_OPTIONS] = {5.0}; // period
    double full_min[TOTAL];
    size_t full_min_len = 0;

    CIndicatorInfo info = min_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u (real), Options: %u (period), Optional outputs: %u\n",
           (unsigned)MIN_INPUTS, (unsigned)MIN_OPTIONS,
           (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", min_min_data(options));

    printf("\n=== MIN: full calculation ===\n");
    {
        const double *inputs[MIN_INPUTS] = {close};

        CIndicatorResult r = min_indicator(inputs, TOTAL, options, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "min_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("min", r.outputs[0], r.output_lens[0]);

        // Keep a copy of the mandatory "min" row for later verification.
        full_min_len = r.output_lens[0];
        for (size_t i = 0; i < full_min_len; i++) full_min[i] = r.outputs[0][i];

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        min_state_free(unused_state);
    }

    printf("\n=== MIN: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[MIN_INPUTS] = {close};
        CIndicatorResult pr = min_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "min_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("min (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[MIN_INPUTS] = {close + PARTIAL};
        CBatchResult br = min_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "min_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("min (continued)", br.outputs[0], br.output_lens[0]);

        printf("\n=== Verification: partial+continued vs. full recompute ===\n");
        int ok = 1;
        size_t continued_len = br.output_lens[0];
        size_t tail_start = full_min_len - continued_len;
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_min[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued min equals full recompute\n"
                  : "  MISMATCH detected!\n");

        tulip_ffi_batch_result_free(br);
        min_state_free(state);
    }

    printf("\n=== MIN: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[MIN_INPUTS] = {close};
        CIndicatorResult pr = min_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "min_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_MIN, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[MIN_INPUTS] = {close + PARTIAL};
        CBatchResult a = min_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = min_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== MIN: SIMD by assets (N=4) ===\n");
    {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            min_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) min_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== MIN: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[MIN_INPUTS] = {close_expanded};

        static const double options_1[MIN_OPTIONS] = {3.0};
        static const double options_2[MIN_OPTIONS] = {5.0};
        static const double options_3[MIN_OPTIONS] = {7.0};
        static const double options_4[MIN_OPTIONS] = {10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at MIN_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = min_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "min_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "min_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
