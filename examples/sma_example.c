// SMA (Simple Moving Average) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_sma_example.py: same sample data,
...
//
// SMA_INPUTS/SMA_OPTIONS come from ../include/tulip_rs_ffi_counts.h,
// generated by ../build.rs from the real tulip_rs::indicators::sma::
...
//   cc -O2 -o sma_example examples/sma_example.c \
...
int main(void) {
    const double options[SMA_OPTIONS] = {5.0}; // period
    double full_sma[TOTAL];
    size_t full_sma_len = 0;

    CIndicatorInfo info = sma_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u, Options: %u (period), Optional outputs: %u\n",
           (unsigned)SMA_INPUTS, (unsigned)SMA_OPTIONS,
           (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", sma_min_data(options));

    printf("\n=== SMA: full calculation ===\n");
    {
        const double *inputs[SMA_INPUTS] = {close};

        CIndicatorResult r = sma_indicator(inputs, TOTAL, options, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "sma_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("sma", r.outputs[0], r.output_lens[0]);

        // Keep a copy of the mandatory "sma" row for later verification.
        full_sma_len = r.output_lens[0];
        for (size_t i = 0; i < full_sma_len; i++) full_sma[i] = r.outputs[0][i];

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        sma_state_free(unused_state);
    }

    printf("\n=== SMA: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[SMA_INPUTS] = {close};
        CIndicatorResult pr = sma_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "sma_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("sma (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[SMA_INPUTS] = {close + PARTIAL};
        CBatchResult br = sma_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "sma_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("sma (continued)", br.outputs[0], br.output_lens[0]);

        printf("\n=== Verification: partial+continued vs. full recompute ===\n");
        int ok = 1;
        size_t continued_len = br.output_lens[0];
        size_t tail_start = full_sma_len - continued_len;
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_sma[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued sma equals full recompute\n"
                  : "  MISMATCH detected!\n");

        tulip_ffi_batch_result_free(br);
        sma_state_free(state);
    }

    printf("\n=== SMA: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[SMA_INPUTS] = {close};
        CIndicatorResult pr = sma_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "sma_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_SMA, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[SMA_INPUTS] = {close + PARTIAL};
        CBatchResult a = sma_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = sma_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== SMA: SIMD by assets (N=4) ===\n");
    {
        ...
        CSimdResult r = sma_simd_by_assets(simd_inputs, 4, TOTAL, options, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "sma_simd_by_assets failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
            printf("Asset %zu ", i + 1);
            print_row("sma", r.outputs[i][0], r.output_lens[i][0]);
        }

        printf("\nVerification - calculating each asset individually:\n");
        int simd_ok = 1;
        for (size_t i = 0; i < r.num_results; i++) {
            CIndicatorResult ind = sma_indicator(
                simd_inputs[i], TOTAL, options, NULL, 0);
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "sma_indicator (asset %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            ...
            sma_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) sma_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== SMA: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[SMA_INPUTS] = {close_expanded};

        static const double options_1[SMA_OPTIONS] = {3.0};
        static const double options_2[SMA_OPTIONS] = {5.0};
        static const double options_3[SMA_OPTIONS] = {7.0};
        static const double options_4[SMA_OPTIONS] = {10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at SMA_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = sma_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "sma_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "sma_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
