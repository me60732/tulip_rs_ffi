...
//
// MACD_INPUTS/MACD_OPTIONS come from tulip_rs_ffi_counts.h, generated by
// ../build.rs from the real tulip_rs::indicators::macd::{INPUTS, OPTIONS}
...
int main(void) {
    const double options[MACD_OPTIONS] = {2.0, 5.0, 9.0}; // short_period, long_period, signal_period
    double full_macd[TOTAL];
    size_t full_macd_len = 0;

    printf("MACD takes %d input series (close) and %d options"
           " (short_period, long_period, signal_period)\n",
           MACD_INPUTS, MACD_OPTIONS);

    printf("\n=== MACD: full calculation (all optional outputs) ===\n");
    {
    ...
    {
        const double *partial_inputs[MACD_INPUTS] = {close};
    ...
    printf("\n=== MACD: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[MACD_INPUTS] = {close};
        bool partial_optional_outputs[2] = {true, true};
        CIndicatorResult pr = macd_indicator(pinputs, PARTIAL, options, partial_optional_outputs, 2);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "macd_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_MACD, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[MACD_INPUTS] = {close + PARTIAL};
        CBatchResult a = macd_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = macd_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== MACD: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[MACD_INPUTS] = {close_expanded};

        static const double options_1[MACD_OPTIONS] = {2.0, 5.0, 9.0};
        static const double options_2[MACD_OPTIONS] = {3.0, 7.0, 11.0};
        static const double options_3[MACD_OPTIONS] = {4.0, 9.0, 13.0};
        static const double options_4[MACD_OPTIONS] = {5.0, 10.0, 15.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at MACD_OPTIONS values.
