// DPO (Detrended Price Oscillator) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_dpo_example.py: same sample data,
...
//
// DPO_INPUTS/DPO_OPTIONS come from ../include/tulip_rs_ffi_counts.h,
// generated by ../build.rs from the real tulip_rs::indicators::dpo::{INPUTS, OPTIONS}
...
//   cc -O2 -o dpo_example examples/dpo_example.c \
...
int main(void) {
    const double options[DPO_OPTIONS] = {5.0}; // period
    double full_dpo[TOTAL];
    size_t full_len = 0;

    CIndicatorInfo info = dpo_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u (real), Options: %u (period)\n",
           (unsigned)DPO_INPUTS, (unsigned)DPO_OPTIONS);
    printf("Optional outputs: %u (sma)\n", (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", dpo_min_data(options));

    printf("\n=== DPO: full calculation (all optional outputs) ===\n");
    {
        const double *inputs[DPO_INPUTS] = {close};
        bool optional_outputs[1] = {true}; // sma

        CIndicatorResult r = dpo_indicator(inputs, TOTAL, options, optional_outputs, 1);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "dpo_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("dpo", r.outputs[0], r.output_lens[0]);
        print_row("sma", r.outputs[1], r.output_lens[1]);

        full_len = r.output_lens[0];
        for (size_t i = 0; i < full_len; i++) {
            full_dpo[i] = r.outputs[0][i];
        }

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        dpo_state_free(unused_state);
    }

    printf("\n=== DPO: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[DPO_INPUTS] = {close};
        CIndicatorResult pr = dpo_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "dpo_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("dpo (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[DPO_INPUTS] = {close + PARTIAL};
        CBatchResult br = dpo_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "dpo_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("dpo (continued)", br.outputs[0], br.output_lens[0]);
        ...
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_dpo[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued DPO equals full recompute\n"
                  : "  MISMATCH detected!\n");
    ...
    printf("\n=== DPO: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[DPO_INPUTS] = {close};
        CIndicatorResult pr = dpo_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "dpo_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_DPO, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[DPO_INPUTS] = {close + PARTIAL};
        CBatchResult a = dpo_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = dpo_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== DPO: SIMD by assets (N=4) ===\n");
    {
        ...
        CSimdResult r = dpo_simd_by_assets(simd_inputs, 4, TOTAL, options, optional_outputs, 1);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "dpo_simd_by_assets failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
            printf("Asset %zu ", i + 1);
            print_row("dpo", r.outputs[i][0], r.output_lens[i][0]);
        }

        printf("\nVerification - calculating each asset individually:\n");
        int simd_ok = 1;
        for (size_t i = 0; i < r.num_results; i++) {
            CIndicatorResult ind = dpo_indicator(
                simd_inputs[i], TOTAL, options, optional_outputs, 1);
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "dpo_indicator (asset %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            ...
            dpo_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) dpo_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== DPO: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[DPO_INPUTS] = {close_expanded};

        static const double options_1[DPO_OPTIONS] = {3.0};
        static const double options_2[DPO_OPTIONS] = {5.0};
        static const double options_3[DPO_OPTIONS] = {7.0};
        static const double options_4[DPO_OPTIONS] = {10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at DPO_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = dpo_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "dpo_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "dpo_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
