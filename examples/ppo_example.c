// PPO (Percentage Price Oscillator) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_ppo_example.py: same sample data,
...
//
// PPO_INPUTS/PPO_OPTIONS come from ../include/tulip_rs_ffi_counts.h,
// generated by ../build.rs from the real tulip_rs::indicators::ppo::
...
//   cc -O2 -o ppo_example examples/ppo_example.c \
...
int main(void) {
    const double options[PPO_OPTIONS] = {2.0, 5.0}; // short_period, long_period
    double full_ppo[TOTAL];
    size_t full_ppo_len = 0;

    CIndicatorInfo info = ppo_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u, Options: %u (short_period, long_period), Optional outputs: %u\n",
           (unsigned)PPO_INPUTS, (unsigned)PPO_OPTIONS,
           (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", ppo_min_data(options));

    printf("\n=== PPO: full calculation (all optional outputs) ===\n");
    {
        const double *inputs[PPO_INPUTS] = {close};
        bool optional_outputs[2] = {true, true}; // short_ema, long_ema

        CIndicatorResult r = ppo_indicator(inputs, TOTAL, options, optional_outputs, 2);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "ppo_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("ppo", r.outputs[0], r.output_lens[0]);
        print_row("short_ema", r.outputs[1], r.output_lens[1]);
        print_row("long_ema", r.outputs[2], r.output_lens[2]);

        // Keep a copy of the mandatory "ppo" row for later verification.
        full_ppo_len = r.output_lens[0];
        for (size_t i = 0; i < full_ppo_len; i++) full_ppo[i] = r.outputs[0][i];

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        ppo_state_free(unused_state);
    }

    printf("\n=== PPO: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[PPO_INPUTS] = {close};
        CIndicatorResult pr = ppo_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "ppo_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("ppo (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[PPO_INPUTS] = {close + PARTIAL};
        CBatchResult br = ppo_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "ppo_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("ppo (continued)", br.outputs[0], br.output_lens[0]);

        printf("\n=== Verification: partial+continued vs. full recompute ===\n");
        int ok = 1;
        size_t continued_len = br.output_lens[0];
        size_t tail_start = full_ppo_len - continued_len;
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_ppo[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued ppo equals full recompute\n"
                  : "  MISMATCH detected!\n");

        tulip_ffi_batch_result_free(br);
        ppo_state_free(state);
    }

    printf("\n=== PPO: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[PPO_INPUTS] = {close};
        CIndicatorResult pr = ppo_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "ppo_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_PPO, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[PPO_INPUTS] = {close + PARTIAL};
        CBatchResult a = ppo_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = ppo_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== PPO: SIMD by assets (N=4) ===\n");
    {
        ...
        CSimdResult r = ppo_simd_by_assets(simd_inputs, 4, TOTAL, options, optional_outputs, 2);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "ppo_simd_by_assets failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
            printf("Asset %zu ", i + 1);
            print_row("ppo", r.outputs[i][0], r.output_lens[i][0]);
        }

        printf("\nVerification - calculating each asset individually:\n");
        int simd_ok = 1;
        for (size_t i = 0; i < r.num_results; i++) {
            CIndicatorResult ind = ppo_indicator(
                simd_inputs[i], TOTAL, options, optional_outputs, 2);
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "ppo_indicator (asset %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            ...
            ppo_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) ppo_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== PPO: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[PPO_INPUTS] = {close_expanded};

        static const double options_1[PPO_OPTIONS] = {1.0, 3.0};
        static const double options_2[PPO_OPTIONS] = {2.0, 5.0};
        static const double options_3[PPO_OPTIONS] = {3.0, 7.0};
        static const double options_4[PPO_OPTIONS] = {4.0, 10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at PPO_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = ppo_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "ppo_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "ppo_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
