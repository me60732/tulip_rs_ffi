...
int main(void) {
    ...
    printf("\n=== TRIX: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[TRIX_INPUTS] = {close};
        CIndicatorResult pr = trix_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "trix_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_TRIX, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[TRIX_INPUTS] = {close + PARTIAL};
        CBatchResult a = trix_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = trix_batch(rs, rinputs, REST, NULL, 0);
