// ROCR (Rate of Change Ratio) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_rocr_example.py: same sample data,
...
//   cc -O2 -o rocr_example examples/rocr_example.c \
...
int main(void) {
    const double options[ROCR_OPTIONS] = {5.0}; // period
    double full_rocr[TOTAL];
    size_t full_rocr_len = 0;

    CIndicatorInfo info = rocr_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u, Options: %u (period), Optional outputs: %u\n",
           (unsigned)ROCR_INPUTS, (unsigned)ROCR_OPTIONS,
           (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", rocr_min_data(options));

    printf("\n=== ROCR: full calculation ===\n");
    {
        const double *inputs[ROCR_INPUTS] = {close};

        CIndicatorResult r = rocr_indicator(inputs, TOTAL, options, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "rocr_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("rocr", r.outputs[0], r.output_lens[0]);

        // Keep a copy of the mandatory "rocr" row for later verification.
        full_rocr_len = r.output_lens[0];
        for (size_t i = 0; i < full_rocr_len; i++) full_rocr[i] = r.outputs[0][i];

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        rocr_state_free(unused_state);
    }

    printf("\n=== ROCR: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[ROCR_INPUTS] = {close};
        CIndicatorResult pr = rocr_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "rocr_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("rocr (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[ROCR_INPUTS] = {close + PARTIAL};
        CBatchResult br = rocr_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "rocr_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("rocr (continued)", br.outputs[0], br.output_lens[0]);

        printf("\n=== Verification: partial+continued vs. full recompute ===\n");
        int ok = 1;
        size_t continued_len = br.output_lens[0];
        size_t tail_start = full_rocr_len - continued_len;
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_rocr[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued rocr equals full recompute\n"
                  : "  MISMATCH detected!\n");

        tulip_ffi_batch_result_free(br);
        rocr_state_free(state);
    }

    printf("\n=== ROCR: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[ROCR_INPUTS] = {close};
        CIndicatorResult pr = rocr_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "rocr_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_ROCR, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[ROCR_INPUTS] = {close + PARTIAL};
        CBatchResult a = rocr_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = rocr_batch(rs, rinputs, REST, NULL, 0);
    ...
    printf("\n=== ROCR: SIMD by assets (N=4) ===\n");
    {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            rocr_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) rocr_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== ROCR: SIMD by options (N=4) ===\n");
    {
        ...
        static const double options_1[ROCR_OPTIONS] = {3.0};
        static const double options_2[ROCR_OPTIONS] = {5.0};
        static const double options_3[ROCR_OPTIONS] = {7.0};
        static const double options_4[ROCR_OPTIONS] = {10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at ROCR_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = rocr_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "rocr_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "rocr_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
