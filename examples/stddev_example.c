...
int main(void) {
    ...
    printf("\n=== StdDev: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[STDDEV_INPUTS] = {close};
        CIndicatorResult pr = stddev_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "stddev_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_STDDEV, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[STDDEV_INPUTS] = {close + PARTIAL};
        CBatchResult a = stddev_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = stddev_batch(rs, rinputs, REST, NULL, 0);
        ...
        stddev_state_free(st);
        stddev_state_free(rs);
    }
