// TEMA (Triple Exponential Moving Average) example for tulip_rs_ffi.
// Mirrors tulip_rs_python/examples/ti_tema_example.py: same sample data,
...
//
// TEMA_INPUTS/TEMA_OPTIONS come from ../include/tulip_rs_ffi_counts.h,
// generated by ../build.rs from the real tulip_rs::indicators::tema::
...
//   cc -O2 -o tema_example examples/tema_example.c \
...
int main(void) {
    const double options[TEMA_OPTIONS] = {5.0}; // period
    double full_tema[TOTAL];
    size_t full_tema_len = 0;

    CIndicatorInfo info = tema_info();
    printf("=== %s (%s) ===\n", info.name, info.full_name);
    printf("Inputs: %u, Options: %u (period), Optional outputs: %u\n",
           (unsigned)TEMA_INPUTS, (unsigned)TEMA_OPTIONS,
           (unsigned)info.optional_outputs.len);
    printf("Minimum data required: %zu\n", tema_min_data(options));

    printf("\n=== TEMA: full calculation (all optional outputs) ===\n");
    {
        const double *inputs[TEMA_INPUTS] = {close};
        bool optional_outputs[2] = {true, true}; // ema, dema

        CIndicatorResult r = tema_indicator(inputs, TOTAL, options, optional_outputs, 2);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "tema_indicator failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_outputs=%zu\n", r.num_outputs);
        print_row("tema", r.outputs[0], r.output_lens[0]);
        print_row("ema", r.outputs[1], r.output_lens[1]);
        print_row("dema", r.outputs[2], r.output_lens[2]);

        // Keep a copy of the mandatory "tema" row for later verification.
        full_tema_len = r.output_lens[0];
        for (size_t i = 0; i < full_tema_len; i++) full_tema[i] = r.outputs[0][i];

        void *unused_state = r.state;
        tulip_ffi_result_free(r);
        tema_state_free(unused_state);
    }

    printf("\n=== TEMA: partial calculation + batch continuation ===\n");
    {
        const double *partial_inputs[TEMA_INPUTS] = {close};
        CIndicatorResult pr = tema_indicator(partial_inputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "tema_indicator (partial) failed: error=%d\n", pr.error);
            return 1;
        }
        print_row("tema (partial)", pr.outputs[0], pr.output_lens[0]);
        void *state = pr.state;
        tulip_ffi_result_free(pr); // outputs freed; state kept alive

        const double *rest_inputs[TEMA_INPUTS] = {close + PARTIAL};
        CBatchResult br = tema_batch(state, rest_inputs, REST, NULL, 0);
        if (br.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "tema_batch failed: error=%d\n", br.error);
            return 1;
        }
        print_row("tema (continued)", br.outputs[0], br.output_lens[0]);

        printf("\n=== Verification: partial+continued vs. full recompute ===\n");
        int ok = 1;
        size_t continued_len = br.output_lens[0];
        size_t tail_start = full_tema_len - continued_len;
        for (size_t i = 0; i < continued_len; i++) {
            double a = full_tema[tail_start + i];
        ...
        printf(ok ? "  MATCH: partial+continued tema equals full recompute\n"
                  : "  MISMATCH detected!\n");

        tulip_ffi_batch_result_free(br);
        tema_state_free(state);
    }

    printf("\n=== TEMA: SIMD by assets (N=4) ===\n");
    {
        // Asset 1: original data.
        const double *const asset1[TEMA_INPUTS] = {close};

        // Asset 2: scaled up (+20%).
        double close_2[TOTAL];
        for (size_t i = 0; i < TOTAL; i++) close_2[i] = close[i] * 1.2;
        const double *const asset2[TEMA_INPUTS] = {close_2};

        // Asset 3: different upward trend.
        double close_3[TOTAL];
        for (size_t i = 0; i < TOTAL; i++)
            close_3[i] = 90.0 + (double)i * 0.5 + close[i] * 0.1;
        const double *const asset3[TEMA_INPUTS] = {close_3};

        // Asset 4: downward trend.
        double close_4[TOTAL];
        for (size_t i = 0; i < TOTAL; i++)
            close_4[i] = 100.0 - (double)i * 0.3 + close[i] * 0.05;
        const double *const asset4[TEMA_INPUTS] = {close_4};
        ...
        CSimdResult r = tema_simd_by_assets(simd_inputs, 4, TOTAL, options, optional_outputs, 2);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "tema_simd_by_assets failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
            printf("Asset %zu ", i + 1);
            print_row("tema", r.outputs[i][0], r.output_lens[i][0]);
        }

        printf("\nVerification - calculating each asset individually:\n");
        int simd_ok = 1;
        for (size_t i = 0; i < r.num_results; i++) {
            CIndicatorResult ind = tema_indicator(
                simd_inputs[i], TOTAL, options, optional_outputs, 2);
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "tema_indicator (asset %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            ...
            tema_state_free(ind_state);
        }

        for (size_t i = 0; i < r.num_results; i++) tema_state_free(r.states[i]);
        tulip_ffi_simd_result_free(r);
        printf(simd_ok ? "  ALL MATCH: SIMD by-assets equals individual calculation\n"
                       : "  MISMATCH detected!\n");
    }

    printf("\n=== TEMA: SIMD by options (N=4) ===\n");
    {
        ...
        const double *expanded_inputs[TEMA_INPUTS] = {close_expanded};

        static const double options_1[TEMA_OPTIONS] = {3.0};
        static const double options_2[TEMA_OPTIONS] = {5.0};
        static const double options_3[TEMA_OPTIONS] = {7.0};
        static const double options_4[TEMA_OPTIONS] = {10.0};
        // simd_options is indexed by option set (the N=4 lanes), each lane
        // pointing at TEMA_OPTIONS values.
        const double *const simd_options[4] = {options_1, options_2, options_3, options_4};

        CSimdResult r = tema_simd_by_options(
            expanded_inputs, EXPANDED_LEN, simd_options, 4, NULL, 0);
        if (r.error != C_INDICATOR_ERROR_OK) {
            fprintf(stderr, "tema_simd_by_options failed: error=%d\n", r.error);
            return 1;
        }
        printf("num_results=%zu, num_outputs=%zu\n", r.num_results, r.num_outputs);
        for (size_t i = 0; i < r.num_results; i++) {
        ...
        for (size_t i = 0; i < r.num_results; i++) {
            ...
            if (ind.error != C_INDICATOR_ERROR_OK) {
                fprintf(stderr, "tema_indicator (option set %zu) failed: error=%d\n", i + 1, ind.error);
                simd_ok = 0;
                continue;
            }
            int match = allclose(r.outputs[i][0], ind.outputs[0], r.output_lens[i][0]);
    ...
    printf("\n=== TEMA: state persistence (serialize / deserialize) ===\n");
    {
        const double *pinputs[TEMA_INPUTS] = {close};
        CIndicatorResult pr = tema_indicator(pinputs, PARTIAL, options, NULL, 0);
        if (pr.error != C_INDICATOR_ERROR_OK) { fprintf(stderr, "tema_indicator failed\n"); return 1; }
        void *st = pr.state;
        tulip_ffi_result_free(pr);

        CBytes blob = tulip_state_serialize(C_INDICATOR_ID_TEMA, C_STATE_FORMAT_BINCODE, st);
        ...
        const double *rinputs[TEMA_INPUTS] = {close + PARTIAL};
        CBatchResult a = tema_batch(st, rinputs, REST, NULL, 0);
        CBatchResult b = tema_batch(rs, rinputs, REST, NULL, 0);
