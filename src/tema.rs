//! extern "C" wrapper for `tema`, mirroring the core `tulip_rs` crate's
//! `Tema::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::tema::Tema::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_tema`'s
...
//!
//! SIMD entry points:
//! - `tema_simd_by_assets`: compute TEMA for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `tema_simd_by_options`: compute TEMA for one asset with N different
...
use tulip_rs::indicators::tema::{IndicatorState as TemaState, Tema, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `tema_indicator()` and consumed by
/// `tema_batch()` / `tema_state_free()`.
pub type TemaStateHandle = TemaState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn tema_info() -> CIndicatorInfo {
    pack_info(&Tema::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn tema_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Tema::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn tema_indicator(
    ...
    match Tema::indicator(&inputs, &options, optional.clone()) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn tema_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut TemaStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn tema_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut TemaStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn tema_simd_by_assets(
    ...
    match num_assets {
        2 => tema_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => tema_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => tema_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => tema_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn tema_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Tema::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn tema_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => tema_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => tema_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => tema_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            tema_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn tema_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Tema::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_tema_indicator() {
        unsafe {
            ...
            let result = tema_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // mandatory=tema only (no optional outputs requested)
    ...
    fn test_tema_batch() {
        unsafe {
            ...
            let batch_result = tema_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            tema_state_free(result.state);
        }
    }
    ...
    fn test_tema_simd_by_assets() {
        unsafe {
            ...
            let result = tema_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
    ...
    #[test]
    fn test_tema_min_data() {
        unsafe {
            let options_arr: [f64; OPTIONS] = [5.0];
            let min = tema_min_data(options_arr.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_tema_simd_by_options() {
        unsafe {
            // Need enough data for max(min_data) across option sets
            // period=5 -> min_data=13, period=10 -> min_data=28
            let data_len = 30;
            ...
            let result = tema_simd_by_options(inputs, data_len, options, 2, ptr::null(), 0);
