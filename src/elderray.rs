//! extern "C" wrapper for `elderray`, mirroring the core `tulip_rs` crate's
//! `Elderray::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::elderray::Elderray::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_elderray`'s
...
//!
//! SIMD entry points:
//! - `elderray_simd_by_assets`: compute Elderray for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `elderray_simd_by_options`: compute Elderray for one asset with N different
...
use tulip_rs::indicators::elderray::{Elderray, IndicatorState as ElderrayState, INPUTS, OPTIONS};
...
/** Opaque state handle returned by `elderray_indicator()` and consumed by
 *   `elderray_batch()` / `elderray_state_free()`.
 */
pub type ElderrayStateHandle = ElderrayState;
 ...
 */
#[no_mangle]
pub extern "C" fn elderray_info() -> CIndicatorInfo {
    pack_info(&Elderray::INFO)
}
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn elderray_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Elderray::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn elderray_indicator(
    ...
    match Elderray::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn elderray_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut ElderrayStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn elderray_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut ElderrayStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn elderray_simd_by_assets(
    ...
    match num_assets {
        2 => {
            elderray_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            elderray_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            elderray_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => elderray_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn elderray_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Elderray::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn elderray_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => elderray_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => elderray_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => elderray_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => elderray_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn elderray_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Elderray::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_elderray_indicator() {
        unsafe {
            ...
            let result = elderray_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // Without optional outputs: only non-empty rows returned (bull, bear)
    ...
    fn test_elderray_batch() {
        unsafe {
            ...
            let new_low_arr: Vec<f64> = (21..=30).map(|x| (x as f64 - 0.5) as f64).collect();
            let new_close_arr: Vec<f64> = (21..=30).map(|x| (x as f64 + 0.25) as f64).collect();
            ...
            let batch_result = elderray_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);
            // Without optional outputs: only non-empty rows returned (bull, bear)
            assert_eq!(batch_result.num_outputs, 2);

            tulip_ffi_batch_result_free(batch_result);
            elderray_state_free(result.state);
        }
    }

    #[test]
    fn test_elderray_simd_by_assets() {
        unsafe {
            let data_len = 20;
            let num_assets = 2;
            ...
            let result =
                elderray_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
            ...
            for i in 0..num_assets {
                elderray_state_free(*result.states.add(i));
            }
    ...
    #[test]
    fn test_elderray_simd_by_options() {
        unsafe {
            let data_len = 20;
            ...
            let result = elderray_simd_by_options(inputs, data_len, options, 2, ptr::null(), 0);
    ...
    #[test]
    fn test_elderray_info() {
        let info = elderray_info();
    ...
    #[test]
    fn test_elderray_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [5.0];
            let min = elderray_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
}
