//! extern "C" wrapper for `rsi`, mirroring the core `tulip_rs` crate's
//! `Rsi::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::rsi::Rsi::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_rsi`'s
...
//!
//! SIMD entry points:
//! - `rsi_simd_by_assets`: compute RSI for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `rsi_simd_by_options`: compute RSI with N different option sets
//!   on a single asset. N must be 2, 4, 8, or 16.

use std::os::raw::c_void;

use tulip_rs::indicator_types::{Indicator, IndicatorByOptions, TIndicatorState};
use tulip_rs::indicators::rsi::{IndicatorState as RsiState, Rsi, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `rsi_indicator()` and consumed by
/// `rsi_batch()` / `rsi_state_free()`.
pub type RsiStateHandle = RsiState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn rsi_info() -> CIndicatorInfo {
    pack_info(&Rsi::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn rsi_min_data(options: *const f64) -> usize {
    let options = std::slice::from_raw_parts(options, OPTIONS);
    Rsi::min_data(&[options[0]])
}
...
#[no_mangle]
pub unsafe extern "C" fn rsi_indicator(
    ...
    let options_ptr = std::slice::from_raw_parts(options, OPTIONS);
    let options_arr: [f64; OPTIONS] = [options_ptr[0]];
    let optional = optional_outputs_slice(optional_outputs, numoptional);

    match Rsi::indicator(&inputs, &options_arr, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn rsi_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut RsiStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn rsi_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut RsiStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn rsi_simd_by_assets(
    ...
    match num_assets {
        2 => rsi_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => rsi_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => rsi_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => rsi_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn rsi_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Rsi::indicator_by_assets::<N>(&refs, &options_arr, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn rsi_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => rsi_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => rsi_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => rsi_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => rsi_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn rsi_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Rsi::indicator_by_options::<N>(&inputs, &simd_options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_rsi_indicator() {
        unsafe {
            ...
            let result = rsi_indicator(inputs, data_len, options, ptr::null(), 0);
    ...
    fn test_rsi_batch() {
        unsafe {
            ...
            let batch_result = rsi_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            rsi_state_free(result.state);
        }
    }
    ...
    fn test_rsi_simd_by_assets() {
        unsafe {
            ...
            let result = rsi_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
    ...
    fn test_rsi_simd_by_options() {
        unsafe {
            ...
            let result =
                rsi_simd_by_options(inputs, data_len, options, num_option_sets, ptr::null(), 0);
    ...
    #[test]
    fn test_rsi_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [14.0];
            let min = rsi_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
}
