//! extern "C" wrapper for `wilders`, mirroring the core `tulip_rs` crate's
//! `Wilders::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::wilders::Wilders::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_wilders`'s
...
//!
//! SIMD entry points:
//! - `wilders_simd_by_assets`: compute WILDERS for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `wilders_simd_by_options`: compute WILDERS with multiple option sets
//!   simultaneously. N must be 2, 4, 8, or 16.

use std::os::raw::c_void;

use tulip_rs::indicator_types::{Indicator, IndicatorByOptions, TIndicatorState};
use tulip_rs::indicators::wilders::{IndicatorState as WildersState, Wilders, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `wilders_indicator()` and consumed by
/// `wilders_batch()` / `wilders_state_free()`.
pub type WildersStateHandle = WildersState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn wilders_info() -> CIndicatorInfo {
    pack_info(&Wilders::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn wilders_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Wilders::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn wilders_indicator(
    ...
    match Wilders::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `data_len` valid `f64`s.
#[no_mangle]
pub unsafe extern "C" fn wilders_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut WildersStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn wilders_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut WildersStateHandle));
    }
}
...
///   `f64`s.
#[no_mangle]
pub unsafe extern "C" fn wilders_simd_by_assets(
    ...
    match num_assets {
        2 => {
            wilders_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            wilders_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            wilders_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => wilders_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn wilders_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Wilders::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
#[no_mangle]
pub unsafe extern "C" fn wilders_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        ...
        16 => wilders_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn wilders_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Wilders::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    unsafe fn min_data(options: &[f64; OPTIONS]) -> usize {
        Wilders::min_data(&[options[0]])
    }

    #[test]
    fn test_wilders_info() {
        let info = wilders_info();
    ...
    #[test]
    fn test_wilders_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = wilders_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_wilders_indicator() {
        unsafe {
            ...
            let result =
                wilders_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);
    ...
    fn test_wilders_batch() {
        unsafe {
            ...
            let batch_result = wilders_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);
            tulip_ffi_batch_result_free(batch_result);
            wilders_state_free(state);
        }
    }

    #[test]
    fn test_wilders_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = wilders_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
    ...
    #[test]
    fn test_wilders_simd_by_options() {
        ...
        unsafe {
            ...
            let result = wilders_simd_by_options(
                inputs.as_ptr(),
                len,
