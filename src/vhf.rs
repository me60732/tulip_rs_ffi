//! extern "C" wrapper for `vhf`, mirroring the core `tulip_rs` crate's
//! `Vhf::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::vhf::Vhf::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_vhf`'s
...
//!
//! SIMD entry points:
//! - `vhf_simd_by_assets`: compute VHF for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `vhf_simd_by_options`: compute VHF for one asset with N different
...
use tulip_rs::indicators::vhf::{IndicatorState as VhfState, Vhf, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `vhf_indicator()` and consumed by
/// `vhf_batch()` / `vhf_state_free()`.
pub type VhfStateHandle = VhfState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn vhf_info() -> CIndicatorInfo {
    pack_info(&Vhf::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn vhf_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Vhf::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn vhf_indicator(
    ...
    match Vhf::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `data_len` valid `f64`s.
#[no_mangle]
pub unsafe extern "C" fn vhf_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut VhfStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn vhf_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut VhfStateHandle));
    }
}
...
/// - `options` must point to `OPTIONS` valid `f64`s.
#[no_mangle]
pub unsafe extern "C" fn vhf_simd_by_assets(
    ...
    match num_assets {
        2 => vhf_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => vhf_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => vhf_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => vhf_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn vhf_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Vhf::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
#[no_mangle]
pub unsafe extern "C" fn vhf_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => vhf_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => vhf_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => vhf_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => vhf_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn vhf_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Vhf::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_vhf_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = vhf_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_vhf_indicator() {
        unsafe {
            ...
            let result = vhf_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only vhf (mandatory)
    ...
    fn test_vhf_batch() {
        unsafe {
            ...
            let batch_result = vhf_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);
    ...
    fn test_vhf_simd_by_options() {
        ...
        unsafe {
            ...
            let result = vhf_simd_by_options(
                inputs.as_ptr(),
                len,
