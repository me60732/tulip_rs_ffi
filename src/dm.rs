//! extern "C" wrapper for `dm`, mirroring the core `tulip_rs` crate's
//! `Dm::indicator` / `IndicatorState::batch_indicator` interface. See
...
//!
//! SIMD entry points:
//! - `dm_simd_by_assets`: compute DM for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `dm_simd_by_options`: compute DM for one asset with N different
...
use tulip_rs::indicators::dm::{Dm, IndicatorState as DmState, INPUTS, OPTIONS};
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn dm_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut DmStateHandle));
    }
}
...
unsafe fn dm_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
...
unsafe fn dm_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
...
mod tests {
    ...
    fn test_dm_indicator() {
        unsafe {
            ...
            // No optional outputs
            let result = dm_indicator(inputs_ptr, high.len(), options_ptr, std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // dm has 2 mandatory outputs (no optional)
    ...
    fn test_dm_batch() {
        unsafe {
            ...
            let batch_result = dm_batch(state, inputs_ptr2, high2.len(), std::ptr::null(), 0);
    ...
    fn test_dm_simd_by_assets() {
        unsafe {
            ...
            let result =
                dm_simd_by_assets(inputs_ptr, 2, high1.len(), options_ptr, std::ptr::null(), 0);
    ...
    fn test_dm_simd_by_options() {
        unsafe {
            ...
            let result =
                dm_simd_by_options(inputs_ptr, high.len(), options_ptr, 2, std::ptr::null(), 0);
