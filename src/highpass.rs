//! extern "C" wrapper for `highpass`, mirroring the core `tulip_rs` crate's
//! `HighPass::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::highpass::HighPass::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_highpass`'s
...
//!
//! SIMD entry points:
//! - `highpass_simd_by_assets`: compute HighPass for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `highpass_simd_by_options`: compute HighPass for one asset with N different
...
use tulip_rs::indicators::highpass::{HighPass, IndicatorState, INPUTS, OPTIONS};
...
/// don't free them.
#[no_mangle]
pub extern "C" fn highpass_info() -> CIndicatorInfo {
    pack_info(&HighPass::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn highpass_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    HighPass::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn highpass_indicator(
    ...
    // highpass has no optional outputs; ignore the request

    let inputs = read_inputs::<INPUTS>(inputs, data_len);
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);

    match HighPass::indicator(&inputs, &options, None) {
        Ok((rows, state)) => {
...
/// - `optional_outputs` is ignored (highpass has no optional outputs).
#[no_mangle]
pub unsafe extern "C" fn highpass_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn highpass_state_free(state: *mut c_void) {
    if !state.is_null() {
...
/// - `optional_outputs` is ignored (highpass has no optional outputs).
#[no_mangle]
pub unsafe extern "C" fn highpass_simd_by_assets(
    ...
    match num_assets {
        ...
        16 => highpass_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn highpass_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match HighPass::indicator_by_assets::<N>(&refs, &options, None) {
        Ok((results, states)) => {
...
/// - `optional_outputs` is ignored (highpass has no optional outputs).
#[no_mangle]
pub unsafe extern "C" fn highpass_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => highpass_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => highpass_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => highpass_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => highpass_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn highpass_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match HighPass::indicator_by_options::<N>(&inputs, &options, None) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    #[test]
    fn test_highpass_info() {
        let info = highpass_info();
    ...
    #[test]
    fn test_highpass_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [20.0];
            let min = highpass_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_highpass_indicator() {
        unsafe {
            let period = 20.0;
            let data_len = 60;
            ...
            // highpass has no optional outputs; pass null + 0
            let result =
                highpass_indicator(inputs.as_ptr(), data_len, options, std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only highpass
    ...
    fn test_highpass_batch() {
        unsafe {
            ...
            // highpass has no optional outputs
            let result =
                highpass_indicator(inputs.as_ptr(), data_len, options, std::ptr::null(), 0);
            ...
            let batch_result = highpass_batch(state, inputs2.as_ptr(), 30, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            highpass_state_free(state);
        }
    }

    #[test]
    fn test_highpass_simd_by_assets() {
        unsafe {
            let period = 20.0;
            let data_len = 60;
            let num_assets = 2;
            ...
            // highpass has no optional outputs
            let result = highpass_simd_by_assets(
                inputs.as_ptr(),
                num_assets,
                data_len,
                options,
    ...
    #[test]
    fn test_highpass_simd_by_options() {
        unsafe {
            let period1 = 20.0;
            let period2 = 30.0;
            let data_len = 60;
            ...
            // highpass has no optional outputs
            let result = highpass_simd_by_options(
                inputs.as_ptr(),
                data_len,
                options.as_ptr(),
                num_option_sets,
