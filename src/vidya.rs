//! extern "C" wrapper for `vidya`, mirroring the core `tulip_rs` crate's
//! `Vidya::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::vidya::Vidya::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_vidya`'s
...
//!
//! SIMD entry points:
//! - `vidya_simd_by_assets`: compute VIDYA for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `vidya_simd_by_options`: compute VIDYA for one asset with N different
...
use tulip_rs::indicators::vidya::{IndicatorState as VidyaState, Vidya, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `vidya_indicator()` and consumed by
/// `vidya_batch()` / `vidya_state_free()`.
pub type VidyaStateHandle = VidyaState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn vidya_info() -> CIndicatorInfo {
    pack_info(&Vidya::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn vidya_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Vidya::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn vidya_indicator(
    ...
    match Vidya::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn vidya_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut VidyaStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn vidya_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut VidyaStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn vidya_simd_by_assets(
    ...
    match num_assets {
        2 => vidya_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => vidya_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => vidya_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            vidya_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn vidya_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Vidya::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn vidya_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => vidya_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => vidya_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => vidya_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            vidya_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn vidya_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Vidya::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    unsafe fn min_data(options: &[f64; OPTIONS]) -> usize {
        Vidya::min_data(&[options[0], options[1], options[2]])
    }
    ...
    #[test]
    fn test_vidya_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [5.0, 20.0, 0.5];
            let min = vidya_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_vidya_indicator() {
        unsafe {
            ...
            let result =
                vidya_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // Without optional outputs: only mandatory rows returned (vidya)
    ...
    fn test_vidya_batch() {
        unsafe {
            ...
            let batch_result = vidya_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);
            tulip_ffi_batch_result_free(batch_result);
            vidya_state_free(state);
        }
    }

    #[test]
    fn test_vidya_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = vidya_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
    ...
    fn test_vidya_simd_by_options() {
        ...
        unsafe {
            ...
            let options1: [f64; OPTIONS] = [5.0, 20.0, 0.5];
            let options2: [f64; OPTIONS] = [10.0, 30.0, 0.3];

            let options_array: [*const f64; NUM_OPTION_SETS] =
                [options1.as_ptr(), options2.as_ptr()];

            let result = vidya_simd_by_options(
                inputs.as_ptr(),
                len,
