//! extern "C" wrapper for `supertrend`, mirroring the core `tulip_rs` crate's
//! `SuperTrend::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::supertrend::SuperTrend::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_supertrend`'s
...
//!
//! SIMD entry points:
//! - `supertrend_simd_by_assets`: compute SuperTrend for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `supertrend_simd_by_options`: compute SuperTrend for one asset with N different
...
use tulip_rs::indicators::supertrend::{
    IndicatorState as SuperTrendState, SuperTrend, INPUTS, OPTIONS,
};
...
/// Opaque state handle returned by `supertrend_indicator()` and consumed by
/// `supertrend_batch()` / `supertrend_state_free()`.
pub type SuperTrendStateHandle = SuperTrendState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn supertrend_info() -> CIndicatorInfo {
    pack_info(&SuperTrend::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn supertrend_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    SuperTrend::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn supertrend_indicator(
    ...
    match SuperTrend::indicator(&inputs, &options, optional.clone()) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn supertrend_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut SuperTrendStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn supertrend_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut SuperTrendStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn supertrend_simd_by_assets(
    ...
    match num_assets {
        2 => supertrend_simd_by_assets_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => supertrend_simd_by_assets_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => supertrend_simd_by_assets_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => supertrend_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn supertrend_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match SuperTrend::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn supertrend_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => supertrend_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => supertrend_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => supertrend_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => supertrend_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn supertrend_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match SuperTrend::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_supertrend_indicator() {
        unsafe {
            ...
            let options_arr: [f64; OPTIONS] = [10.0, 3.0];
            let options: *const f64 = options_arr.as_ptr();

            let result = supertrend_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // mandatory=supertrend only (no optional outputs requested)
    ...
    fn test_supertrend_batch() {
        unsafe {
            ...
            let new_close_arr: Vec<f64> = build_synthetic_data(data_len);
            ...
            let batch_result = supertrend_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            supertrend_state_free(result.state);
        }
    }

    #[test]
    fn test_supertrend_simd_by_assets() {
        unsafe {
            let data_len = 20;
            let num_assets = 2;
            ...
            let result =
                supertrend_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
    ...
    #[test]
    fn test_supertrend_info() {
        let info = supertrend_info();
    ...
    #[test]
    fn test_supertrend_min_data() {
        unsafe {
            let options_arr: [f64; OPTIONS] = [10.0, 3.0];
            let min = supertrend_min_data(options_arr.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_supertrend_simd_by_options() {
        unsafe {
            let data_len = 20;
            ...
            let options0: [f64; OPTIONS] = [10.0, 3.0];
            let options1: [f64; OPTIONS] = [14.0, 2.5];
            let options_arr: [*const f64; 2] = [options0.as_ptr(), options1.as_ptr()];
            let options: *const *const f64 = options_arr.as_ptr();

            let result = supertrend_simd_by_options(inputs, data_len, options, 2, ptr::null(), 0);
