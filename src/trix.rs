//! extern "C" wrapper for `trix`, mirroring the core `tulip_rs` crate's
//! `Trix::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::trix::Trix::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_trix`'s
...
//!
//! SIMD entry points:
//! - `trix_simd_by_assets`: compute TRIX for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `trix_simd_by_options`: compute TRIX for one asset with N different
...
use tulip_rs::indicators::trix::{IndicatorState as TrixState, Trix, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `trix_indicator()` and consumed by
/// `trix_batch()` / `trix_state_free()`.
pub type TrixStateHandle = TrixState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn trix_info() -> CIndicatorInfo {
    pack_info(&Trix::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn trix_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Trix::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn trix_indicator(
    ...
    match Trix::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn trix_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut TrixStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn trix_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut TrixStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn trix_simd_by_assets(
    ...
    match num_assets {
        2 => trix_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => trix_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => trix_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => trix_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn trix_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Trix::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn trix_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => trix_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => trix_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => trix_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            trix_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn trix_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Trix::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_trix_indicator() {
        unsafe {
            ...
            // Request all optional outputs (tema, dema, ema)
            let optional_arr: [bool; 3] = [true, true, true];
            let optional: *const bool = optional_arr.as_ptr();

            let result = trix_indicator(inputs, data_len, options, optional, 3);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 4); // trix + tema + dema + ema
    ...
    fn test_trix_indicator_mandatory_only() {
        unsafe {
            ...
            let result = trix_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only trix
            assert!(!result.state.is_null());
    ...
    fn test_trix_batch() {
        unsafe {
            ...
            let batch_result = trix_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            trix_state_free(result.state);
        }
    }
    ...
    fn test_trix_simd_by_assets() {
        unsafe {
            ...
            let result = trix_simd_by_assets(inputs, num_assets, data_len, options, optional, 3);
    ...
    fn test_trix_info() {
        ...
        assert_eq!(info.outputs.len, 1); // mandatory: trix
        assert_eq!(info.optional_outputs.len, 3); // tema, dema, ema
    }

    #[test]
    fn test_trix_min_data() {
        unsafe {
            let options_arr: [f64; OPTIONS] = [5.0];
            let min = trix_min_data(options_arr.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_trix_simd_by_options() {
        unsafe {
            let data_len = 30;
            ...
            let result = trix_simd_by_options(inputs, data_len, options, 2, optional, 3);
