//! extern "C" wrapper for `ema`, mirroring the core `tulip_rs` crate's
//! `Ema::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::ema::Ema::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_ema`'s
...
//!
//! SIMD entry points:
//! - `ema_simd_by_assets`: compute EMA for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `ema_simd_by_options`: compute EMA for one asset with N different
...
use tulip_rs::indicators::ema::{Ema, IndicatorState as EmaState, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `ema_indicator()` and consumed by
/// `ema_batch()` / `ema_state_free()`.
pub type EmaStateHandle = EmaState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn ema_info() -> CIndicatorInfo {
    pack_info(&Ema::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn ema_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Ema::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn ema_indicator(
    ...
    match Ema::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ema_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut EmaStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn ema_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut EmaStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ema_simd_by_assets(
    ...
    match num_assets {
        2 => ema_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => ema_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => ema_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => ema_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn ema_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Ema::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ema_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => ema_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => ema_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => ema_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            ema_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn ema_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Ema::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    fn test_ema_indicator() {
        unsafe {
            ...
            let result = ema_indicator(inputs, data_len, options, ptr::null(), 0);
    ...
    fn test_ema_batch() {
        unsafe {
            ...
            let batch_result = ema_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            ema_state_free(result.state);
        }
    }

    #[test]
    fn test_ema_simd_by_assets() {
        unsafe {
            let data_len = 20;
            let num_assets = 2;
            ...
            let result = ema_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
    ...
    #[test]
    fn test_ema_min_data() {
        unsafe {
            let options_arr: [f64; OPTIONS] = [5.0];
            let min = ema_min_data(options_arr.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_ema_simd_by_options() {
        unsafe {
            let data_len = 20;
            ...
            let result = ema_simd_by_options(inputs, data_len, options, 2, ptr::null(), 0);
