//! extern "C" wrapper for `donchianchannel`, mirroring the core `tulip_rs` crate's
//! `DonchianChannel::indicator` / `IndicatorState::batch_indicator` interface. See
...
//!
//! SIMD entry points:
//! - `donchianchannel_simd_by_assets`: compute Donchian Channel for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `donchianchannel_simd_by_options`: compute Donchian Channel for one asset with N different
...
use tulip_rs::indicators::donchianchannel::{
    DonchianChannel, IndicatorState as DonchianChannelState, INPUTS, OPTIONS,
};
...
/// Opaque state handle returned by `donchianchannel_indicator()` and consumed by
/// `donchianchannel_batch()` / `donchianchannel_state_free().
pub type DonchianChannelStateHandle = DonchianChannelState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn donchianchannel_info() -> CIndicatorInfo {
    pack_info(&DonchianChannel::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    DonchianChannel::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_indicator(
    ...
    match DonchianChannel::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut DonchianChannelStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut DonchianChannelStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_simd_by_assets(
    ...
    match num_assets {
        2 => donchianchannel_simd_by_assets_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => donchianchannel_simd_by_assets_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => donchianchannel_simd_by_assets_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => donchianchannel_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn donchianchannel_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match DonchianChannel::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn donchianchannel_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => donchianchannel_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => donchianchannel_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => donchianchannel_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => donchianchannel_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn donchianchannel_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match DonchianChannel::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    #[test]
    fn test_donchianchannel_info() {
        let info = donchianchannel_info();
    ...
    #[test]
    fn test_donchianchannel_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [20.0];
            let min = donchianchannel_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_donchianchannel_indicator() {
        unsafe {
            // Create test data: high, low prices
            ...
            // No optional outputs
            let result =
                donchianchannel_indicator(inputs_ptr, high.len(), options_ptr, std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // donchianchannel has 3 mandatory outputs (no optional)
    ...
    fn test_donchianchannel_batch() {
        unsafe {
            ...
            let batch_result =
                donchianchannel_batch(state, inputs_ptr2, high2.len(), std::ptr::null(), 0);
    ...
    #[test]
    fn test_donchianchannel_simd_by_assets() {
        unsafe {
            // Two assets, each with high, low prices
            ...
            let result = donchianchannel_simd_by_assets(
                inputs_ptr,
                2,
                high1.len(),
                options_ptr,
    ...
    #[test]
    fn test_donchianchannel_simd_by_options() {
        unsafe {
            // Single asset with high, low prices (need enough data for period=5)
            ...
            let result = donchianchannel_simd_by_options(
                inputs_ptr,
                high.len(),
                options_ptr,
                2,
