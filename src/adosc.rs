...
// The C-visible `ADOSC_INPUTS`/`ADOSC_OPTIONS` counts are generated by
// `build.rs` (which reads these same `tulip_rs::indicators::adosc::{INPUTS,
// OPTIONS}` constants at build time) into `tulip_rs_ffi_counts.h`, so they
// can never drift from the core crate. No per-module Rust const is needed
