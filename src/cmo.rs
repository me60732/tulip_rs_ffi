//! extern "C" wrapper for `cmo`, mirroring the core `tulip_rs` crate's
//! `Cmo::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::cmo::Cmo::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_cmo`'s
...
//!
//! SIMD entry points:
//! - `cmo_simd_by_assets`: compute CMO for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `cmo_simd_by_options`: compute CMO for one asset with N different
...
use tulip_rs::indicators::cmo::{Cmo, IndicatorState as CmoState, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `cmo_indicator()` and consumed by
/// `cmo_batch()` / `cmo_state_free().
pub type CmoStateHandle = CmoState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn cmo_info() -> CIndicatorInfo {
    pack_info(&Cmo::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn cmo_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Cmo::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn cmo_indicator(
    ...
    match Cmo::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cmo_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut CmoStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn cmo_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut CmoStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cmo_simd_by_assets(
    ...
    match num_assets {
        2 => cmo_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => cmo_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => cmo_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => cmo_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn cmo_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Cmo::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cmo_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => cmo_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => cmo_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => cmo_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => cmo_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn cmo_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Cmo::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_cmo_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = cmo_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_cmo_simd_by_options() {
        ...
        unsafe {
            let result = cmo_simd_by_options(
                inputs.as_ptr(),
                data_len,
