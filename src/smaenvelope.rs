//! extern "C" wrapper for `smaenvelope`, mirroring the core `tulip_rs` crate's
//! `SmaEnvelope::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::smaenvelope::SmaEnvelope::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_smaenvelope`'s
...
//!
//! SIMD entry points:
//! - `smaenvelope_simd_by_assets`: compute SMA Envelope for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `smaenvelope_simd_by_options`: compute SMA Envelope with N different option sets
...
use tulip_rs::indicators::smaenvelope::{
    IndicatorState as SmaEnvelopeState, SmaEnvelope, INPUTS, OPTIONS,
};
...
/// Opaque state handle returned by `smaenvelope_indicator()` and consumed by
/// `smaenvelope_batch()` / `smaenvelope_state_free()`.
pub type SmaEnvelopeStateHandle = SmaEnvelopeState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn smaenvelope_info() -> CIndicatorInfo {
    pack_info(&SmaEnvelope::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_min_data(options: *const f64) -> usize {
    let options_arr: [f64; OPTIONS] = std::slice::from_raw_parts(options, OPTIONS)
        .try_into()
        .unwrap();
    SmaEnvelope::min_data(&options_arr)
}
...
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_indicator(
    ...
    match SmaEnvelope::indicator(&inputs, &options_arr, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut SmaEnvelopeStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut SmaEnvelopeStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_simd_by_assets(
    ...
    match num_assets {
        2 => smaenvelope_simd_by_assets_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => smaenvelope_simd_by_assets_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => smaenvelope_simd_by_assets_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => smaenvelope_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn smaenvelope_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match SmaEnvelope::indicator_by_assets::<N>(&refs, &options_arr, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn smaenvelope_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => smaenvelope_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => smaenvelope_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => smaenvelope_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => smaenvelope_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn smaenvelope_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match SmaEnvelope::indicator_by_options::<N>(&inputs, &simd_options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_smaenvelope_indicator() {
        unsafe {
            ...
            let options_arr: [f64; OPTIONS] = [14.0, 10.0];
            let options: *const f64 = options_arr.as_ptr();

            let result = smaenvelope_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 3); // lower, middle, upper
    ...
    fn test_smaenvelope_batch() {
        unsafe {
            ...
            let batch_result = smaenvelope_batch(result.state, new_inputs, 10, ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            tulip_ffi_batch_result_free(batch_result);
            smaenvelope_state_free(result.state);
        }
    }

    #[test]
    fn test_smaenvelope_simd_by_assets() {
        unsafe {
            let data_len = 60;
            let num_assets = 4;
            ...
            let result =
                smaenvelope_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);
    ...
    #[test]
    fn test_smaenvelope_simd_by_options() {
        unsafe {
            let data_len = 60;
    ...
    #[test]
    fn test_smaenvelope_info() {
        let info = smaenvelope_info();
    ...
    #[test]
    fn test_smaenvelope_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [14.0, 10.0];
            let min = smaenvelope_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
}
