//! extern "C" wrapper for `linreg`, mirroring the core `tulip_rs` crate's
//! `Linreg::indicator` / `IndicatorState::batch_indicator` interface with a
...
//!
//! SIMD entry points:
//! - `linreg_simd_by_assets`: compute LINREG for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `linreg_simd_by_options`: compute LINREG for one asset with N different
...
use tulip_rs::indicators::linreg::{IndicatorState as LinRegState, Linreg, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `linreg_indicator()` and consumed by
/// `linreg_batch()` / `linreg_state_free()`.
pub type LinRegStateHandle = LinRegState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn linreg_info() -> CIndicatorInfo {
    pack_info(&Linreg::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn linreg_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Linreg::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn linreg_indicator(
    ...
    match Linreg::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn linreg_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut LinRegStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn linreg_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut LinRegStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn linreg_simd_by_assets(
    ...
    match num_assets {
        2 => {
            linreg_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            linreg_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            linreg_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => {
            linreg_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn linreg_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Linreg::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn linreg_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => {
            linreg_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            linreg_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            linreg_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => linreg_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn linreg_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Linreg::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    #[test]
    fn test_linreg_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [20.0];
            let min = linreg_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_linreg_indicator() {
        unsafe {
            ...
            let result =
                linreg_indicator(inputs, data_len, options_ptr, optional_ptr, numoptional);

            assert_eq!(result.error, CIndicatorError::Ok);
            // linreg has 3 outputs: linreg, linregslope, linregintercept
    ...
    fn test_linreg_batch() {
        unsafe {
            ...
            let result = linreg_indicator(inputs, data_len, options_ptr, std::ptr::null(), 0);
            assert_eq!(result.error, CIndicatorError::Ok);
            ...
            let batch_result = linreg_batch(state, inputs2, data_len, std::ptr::null(), 0);
    ...
    fn test_linreg_simd_by_assets() {
        unsafe {
            ...
            let result = linreg_simd_by_assets(
                inputs,
                num_assets,
                data_len,
                options_ptr,
    ...
    #[test]
    fn test_linreg_simd_by_options() {
        unsafe {
            ...
            let result = linreg_simd_by_options(
                inputs,
                data_len,
                options,
                num_option_sets,
