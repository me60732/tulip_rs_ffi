//! extern "C" wrapper for `keltnerchannel`, mirroring the core `tulip_rs` crate's
//! `KeltnerChannel::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::keltnerchannel::KeltnerChannel::INFO.inputs`),
...
//!
//! SIMD entry points:
//! - `keltnerchannel_simd_by_assets`: compute KeltnerChannel for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `keltnerchannel_simd_by_options`: compute KeltnerChannel for one asset with N different
...
use tulip_rs::indicators::keltnerchannel::{
    IndicatorState as KeltnerChannelState, KeltnerChannel, INPUTS, OPTIONS,
};
...
/// Opaque state handle returned by `keltnerchannel_indicator()` and consumed by
/// `keltnerchannel_batch()` / `keltnerchannel_state_free()`.
pub type KeltnerChannelStateHandle = KeltnerChannelState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn keltnerchannel_info() -> CIndicatorInfo {
    pack_info(&KeltnerChannel::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    KeltnerChannel::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_indicator(
    ...
    match KeltnerChannel::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut KeltnerChannelStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut KeltnerChannelStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_simd_by_assets(
    ...
    match num_assets {
        2 => keltnerchannel_simd_by_assets_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => keltnerchannel_simd_by_assets_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => keltnerchannel_simd_by_assets_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => keltnerchannel_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn keltnerchannel_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match KeltnerChannel::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn keltnerchannel_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => keltnerchannel_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => keltnerchannel_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => keltnerchannel_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => keltnerchannel_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn keltnerchannel_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match KeltnerChannel::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_keltnerchannel_info() {
        let info = keltnerchannel_info();
    ...
    #[test]
    fn test_keltnerchannel_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [20.0, 2.0];
            let min = keltnerchannel_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_keltnerchannel_indicator() {
        unsafe {
            let options = [20.0, 2.0];
            ...
            let result = keltnerchannel_indicator(
                inputs.as_ptr(),
                len,
                options.as_ptr(),
                std::ptr::null(),
            ...
            assert_eq!(result.num_outputs, 3); // only lower, middle, upper (mandatory)
    ...
    fn test_keltnerchannel_batch() {
        unsafe {
            ...
            let batch_result =
                keltnerchannel_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);
    ...
    #[test]
    fn test_keltnerchannel_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = keltnerchannel_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
            ...
            for i in 0..NUM_ASSETS {
                ...
                keltnerchannel_state_free(*state_ptr);
            }
    ...
    #[test]
    fn test_keltnerchannel_simd_by_options() {
        const NUM_OPTION_SETS: usize = 2;
        unsafe {
            // Must satisfy min_data for *all* option sets used below.
            let len = min_data(&[20.0, 2.0]).max(min_data(&[25.0, 2.5]));
            ...
            let options2: [f64; OPTIONS] = [25.0, 2.5];

            let options_array: [*const f64; NUM_OPTION_SETS] =
                [options1.as_ptr(), options2.as_ptr()];

            let result = keltnerchannel_simd_by_options(
                inputs.as_ptr(),
                len,
