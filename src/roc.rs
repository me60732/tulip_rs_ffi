//! extern "C" wrapper for `roc`, mirroring the core `tulip_rs` crate's
//! `Roc::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::roc::Roc::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_roc`'s
...
//!
//! SIMD entry points:
//! - `roc_simd_by_assets`: compute ROC for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `roc_simd_by_options`: compute ROC for N option sets simultaneously
...
use tulip_rs::indicators::roc::{IndicatorState as RocState, Roc, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `roc_indicator()` and consumed by
/// `roc_batch()` / `roc_state_free()`.
pub type RocStateHandle = RocState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn roc_info() -> CIndicatorInfo {
    pack_info(&Roc::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn roc_min_data(options: *const f64) -> usize {
    let options = std::slice::from_raw_parts(options, OPTIONS);
    Roc::min_data(&options.try_into().unwrap())
}
...
#[no_mangle]
pub unsafe extern "C" fn roc_indicator(
    ...
    match Roc::indicator(&inputs, &options_array, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn roc_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut RocStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn roc_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut RocStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn roc_simd_by_assets(
    ...
    match num_assets {
        2 => roc_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => roc_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => roc_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => roc_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn roc_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Roc::indicator_by_assets::<N>(&refs, &options_array, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn roc_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => roc_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => roc_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => roc_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => roc_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn roc_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Roc::indicator_by_options::<N>(&inputs, &simd_options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    fn test_roc_indicator() {
        unsafe {
            ...
            // Request optional output (mom)
            let optional_arr: [bool; 1] = [true];
            let optional: *const bool = optional_arr.as_ptr();

            let result = roc_indicator(inputs, data_len, options, optional, 1);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 2); // roc + mom
    ...
    #[test]
    fn test_roc_indicator_nooptional() {
        unsafe {
            let data_len = 60;
            let real_arr: Vec<f64> = build_synthetic_data(data_len);
            ...
            let result = roc_indicator(inputs, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only roc
            assert!(!result.state.is_null());
    ...
    fn test_roc_simd_by_assets() {
        unsafe {
            ...
            let result = roc_simd_by_assets(inputs, num_assets, data_len, options, ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_results, 4);
            assert_eq!(result.num_outputs, 1); // only roc (mandatory)
    ...
    fn test_roc_simd_by_options() {
        unsafe {
            ...
            // Two different option sets (the `period` value).
            ...
            // Request the one optional output (`mom`).
            let optional_arr: [bool; 1] = [true];
            let optional: *const bool = optional_arr.as_ptr();

            let result = roc_simd_by_options(inputs, data_len, options, 2, optional, 1);
    ...
    #[test]
    fn test_roc_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = roc_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
}
