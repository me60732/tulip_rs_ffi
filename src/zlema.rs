//! extern "C" wrapper for `zlema`, mirroring the core `tulip_rs` crate's
//! `Zlema::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::zlema::Zlema::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_zlema`'s
...
//!
//! SIMD entry points:
//! - `zlema_simd_by_assets`: compute ZLEMA for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `zlema_simd_by_options`: compute ZLEMA for one asset with N different
...
use tulip_rs::indicators::zlema::{IndicatorState as ZlemaState, Zlema, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `zlema_indicator()` and consumed by
/// `zlema_batch()` / `zlema_state_free()`.
pub type ZlemaStateHandle = ZlemaState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn zlema_info() -> CIndicatorInfo {
    pack_info(&Zlema::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn zlema_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Zlema::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn zlema_indicator(
    ...
    match Zlema::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `data_len` valid `f64`s.
#[no_mangle]
pub unsafe extern "C" fn zlema_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut ZlemaStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn zlema_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut ZlemaStateHandle));
    }
}
...
/// - `options` must point to `OPTIONS` valid `f64`s.
#[no_mangle]
pub unsafe extern "C" fn zlema_simd_by_assets(
    ...
    match num_assets {
        2 => zlema_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => zlema_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => zlema_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            zlema_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn zlema_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Zlema::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
#[no_mangle]
pub unsafe extern "C" fn zlema_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => zlema_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => zlema_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => zlema_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            zlema_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn zlema_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Zlema::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_zlema_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = zlema_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_zlema_indicator() {
        unsafe {
            ...
            let result =
                zlema_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only zlema (mandatory)
    ...
    fn test_zlema_batch() {
        unsafe {
            ...
            let batch_result = zlema_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);
            tulip_ffi_batch_result_free(batch_result);
            zlema_state_free(state);
        }
    }

    #[test]
    fn test_zlema_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = zlema_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
    ...
    fn test_zlema_simd_by_options() {
        ...
        unsafe {
            ...
            let result = zlema_simd_by_options(
                inputs.as_ptr(),
                len,
