//! extern "C" wrapper for `cci`, mirroring the core `tulip_rs` crate's
//! `Cci::indicator` / `IndicatorState::batch_indicator` interface. See
...
//!
//! SIMD entry points:
//! - `cci_simd_by_assets`: compute CCI for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `cci_simd_by_options`: compute CCI for one asset with N different
...
use tulip_rs::indicators::cci::{Cci, IndicatorState as CciState, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `cci_indicator()` and consumed by
/// `cci_batch()` / `cci_state_free().
pub type CciStateHandle = CciState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn cci_info() -> CIndicatorInfo {
    pack_info(&Cci::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn cci_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Cci::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn cci_indicator(
    ...
    match Cci::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cci_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut CciStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn cci_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut CciStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cci_simd_by_assets(
    ...
    match num_assets {
        2 => cci_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => cci_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => cci_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => cci_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn cci_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Cci::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn cci_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => cci_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => cci_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => cci_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => cci_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn cci_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Cci::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_cci_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [20.0];
            let min = cci_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_cci_indicator() {
        unsafe {
            // Create test data - need at least period * 2 - 1 bars
            let data_len = 100;
            ...
            // Without optional outputs: only mandatory rows returned (cci)
            ...
            assert_eq!(lens[0], 62); // cci (mandatory)
    ...
    fn test_cci_batch() {
        unsafe {
            ...
            let new_inputs = [more_high.as_ptr(), more_low.as_ptr(), more_close.as_ptr()];

            let batch_result =
                cci_batch(result.state, new_inputs.as_ptr(), 10, std::ptr::null(), 0);
            assert_eq!(batch_result.error, CIndicatorError::Ok);

            // Free outputs
            tulip_ffi_batch_result_free(batch_result);
            cci_state_free(result.state);
        }
    }

    #[test]
    fn test_cci_simd_by_assets() {
        unsafe {
            let data_len = 100;
            let num_assets = 2;
    ...
    fn test_cci_simd_by_options() {
        unsafe {
            ...
            let result = cci_simd_by_options(
                inputs.as_ptr(),
                data_len,
                option_sets.as_ptr(),
                num_option_sets,
