//! extern "C" wrapper for `aroonosc`, mirroring the core `tulip_rs` crate's
//! `AroonOsc::indicator` / `IndicatorState::batch_indicator` interface. See
...
//!
//! SIMD entry points:
//! - `aroonosc_simd_by_assets`: compute AROONOSC for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `aroonosc_simd_by_options`: compute AROONOSC for one asset with N different
...
use tulip_rs::indicators::aroonosc::{AroonOsc, IndicatorState as AroonOscState, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `aroonosc_indicator()` and consumed by
/// `aroonosc_batch()` / `aroonosc_state_free()`.
pub type AroonOscStateHandle = AroonOscState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn aroonosc_info() -> CIndicatorInfo {
    pack_info(&AroonOsc::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn aroonosc_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    AroonOsc::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn aroonosc_indicator(
    ...
    match AroonOsc::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn aroonosc_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut AroonOscStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn aroonosc_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut AroonOscStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn aroonosc_simd_by_assets(
    ...
    match num_assets {
        2 => {
            aroonosc_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            aroonosc_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            aroonosc_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => aroonosc_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn aroonosc_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match AroonOsc::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn aroonosc_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => aroonosc_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => aroonosc_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => aroonosc_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => aroonosc_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn aroonosc_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match AroonOsc::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_aroonosc_info() {
        let info = aroonosc_info();
    ...
    #[test]
    fn test_aroonosc_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [3.0];
            let min = aroonosc_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_aroonosc_indicator() {
        unsafe {
            ...
            // No optional outputs
            let result =
                aroonosc_indicator(inputs_ptr, high.len(), options_ptr, std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // Without optional outputs: only mandatory rows returned (aroonosc)
    ...
    fn test_aroonosc_batch() {
        unsafe {
            ...
            let batch_result = aroonosc_batch(state, inputs_ptr2, high2.len(), std::ptr::null(), 0);
    ...
    #[test]
    fn test_aroonosc_simd_by_assets() {
        unsafe {
            ...
            let result = aroonosc_simd_by_assets(
                inputs_ptr,
                2,
                high1.len(),
                options_ptr,
    ...
    #[test]
    fn test_aroonosc_simd_by_options() {
        unsafe {
            ...
            let result = aroonosc_simd_by_options(
                inputs_ptr,
                high.len(),
                options_ptr,
                2,
