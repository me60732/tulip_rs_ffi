//! extern "C" wrapper for `kama`, mirroring the core `tulip_rs` crate's
//! `Kama::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::kama::Kama::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_kama`'s
...
//!
//! SIMD entry points:
//! - `kama_simd_by_assets`: compute KAMA for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `kama_simd_by_options`: compute KAMA for one asset with N different
...
use tulip_rs::indicators::kama::{IndicatorState as KamaState, Kama, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `kama_indicator()` and consumed by
/// `kama_batch()` / `kama_state_free()`.
pub type KamaStateHandle = KamaState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn kama_info() -> CIndicatorInfo {
    pack_info(&Kama::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn kama_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Kama::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn kama_indicator(
    ...
    match Kama::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn kama_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut KamaStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn kama_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut KamaStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn kama_simd_by_assets(
    ...
    match num_assets {
        2 => kama_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => kama_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => kama_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => kama_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn kama_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Kama::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn kama_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => kama_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => kama_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => kama_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            kama_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn kama_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Kama::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_kama_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [10.0];
            let min = kama_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_kama_indicator() {
        unsafe {
            ...
            let result =
                kama_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            assert_eq!(result.num_outputs, 1); // only kama (mandatory)
    ...
    fn test_kama_batch() {
        unsafe {
            ...
            let batch_result = kama_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);
            tulip_ffi_batch_result_free(batch_result);
            kama_state_free(state);
        }
    }

    #[test]
    fn test_kama_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = kama_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
    ...
    fn test_kama_simd_by_options() {
        ...
        unsafe {
            ...
            let result = kama_simd_by_options(
                inputs.as_ptr(),
                len,
