...
/// don't free them.
#[no_mangle]
pub extern "C" fn dx_info() -> CIndicatorInfo {
    pack_info(&Dx::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn dx_min_data(options: *const f64) -> usize {
...
pub unsafe extern "C" fn dx_simd_by_assets(
    ...
    match num_assets {
        2 => dx_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => dx_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => dx_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => dx_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
...
pub unsafe extern "C" fn dx_simd_by_options(
    ...
    match num_option_sets {
        2 => dx_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => dx_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => dx_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => dx_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}
...
mod tests {
    ...
    fn test_dx_min_data() {
        unsafe {
            ...
            let min = dx_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }
    ...
    fn test_dx_indicator() {
        unsafe {
            ...
            // Without optional outputs: only mandatory rows returned (dx)
