//! extern "C" wrapper for `ichimoku`, mirroring the core `tulip_rs` crate's
//! `Ichimoku::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::ichimoku::Ichimoku::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_ichimoku`'s
...
//!
//! SIMD entry points:
//! - `ichimoku_simd_by_assets`: compute Ichimoku for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `ichimoku_simd_by_options`: compute Ichimoku for one asset with N different
...
use tulip_rs::indicators::ichimoku::{Ichimoku, IndicatorState as IchimokuState, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `ichimoku_indicator()` and consumed by
/// `ichimoku_batch()` / `ichimoku_state_free()`.
pub type IchimokuStateHandle = IchimokuState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn ichimoku_info() -> CIndicatorInfo {
    pack_info(&Ichimoku::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn ichimoku_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Ichimoku::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn ichimoku_indicator(
    ...
    match Ichimoku::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ichimoku_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut IchimokuStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn ichimoku_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut IchimokuStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ichimoku_simd_by_assets(
    ...
    match num_assets {
        2 => {
            ichimoku_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional)
        }
        4 => {
            ichimoku_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional)
        }
        8 => {
            ichimoku_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional)
        }
        16 => ichimoku_simd_by_assets_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn ichimoku_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Ichimoku::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn ichimoku_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => ichimoku_simd_by_options_n::<2>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        4 => ichimoku_simd_by_options_n::<4>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        8 => ichimoku_simd_by_options_n::<8>(
            inputs,
            data_len,
            options,
            optional_outputs,
        ...
        16 => ichimoku_simd_by_options_n::<16>(
            inputs,
            data_len,
            options,
            optional_outputs,
...
unsafe fn ichimoku_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Ichimoku::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) =
                pack_simd_outputs(results, optional);
            let states = pack_states(states);
...
mod tests {
    ...
    #[test]
    fn test_ichimoku_info() {
        let info = ichimoku_info();
    ...
    #[test]
    fn test_ichimoku_min_data() {
        unsafe {
            let options: [f64; OPTIONS] = [9.0, 26.0];
            let min = ichimoku_min_data(options.as_ptr());
            assert!(min > 0);
        }
    }

    #[test]
    fn test_ichimoku_indicator() {
        unsafe {
            let options = [9.0, 26.0];
            ...
            let result =
                ichimoku_indicator(inputs.as_ptr(), len, options.as_ptr(), std::ptr::null(), 0);

            assert_eq!(result.error, CIndicatorError::Ok);
            // Without optional outputs: only non-empty rows returned (conversion, base, leading_span_a, leading_span_b)
    ...
    fn test_ichimoku_batch() {
        unsafe {
            ...
            let batch_result = ichimoku_batch(state, inputs2.as_ptr(), len2, std::ptr::null(), 0);

            assert_eq!(batch_result.error, CIndicatorError::Ok);
            tulip_ffi_batch_result_free(batch_result);
            ichimoku_state_free(state);
        }
    }
    ...
    fn test_ichimoku_simd_by_assets() {
        ...
        unsafe {
            ...
            let result = ichimoku_simd_by_assets(
                inputs.as_ptr(),
                NUM_ASSETS,
                len,
                options.as_ptr(),
    ...
    fn test_ichimoku_simd_by_options() {
        ...
        unsafe {
            ...
            let len = min_data(&[9.0, 26.0]).max(min_data(&[12.0, 30.0]));
            ...
            let options1: [f64; OPTIONS] = [9.0, 26.0];
            let options2: [f64; OPTIONS] = [12.0, 30.0];

            let options_array: [*const f64; NUM_OPTION_SETS] =
                [options1.as_ptr(), options2.as_ptr()];

            let result = ichimoku_simd_by_options(
                inputs.as_ptr(),
                len,
