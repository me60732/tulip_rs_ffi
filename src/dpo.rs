//! extern "C" wrapper for `dpo`, mirroring the core `tulip_rs` crate's
//! `Dpo::indicator` / `IndicatorState::batch_indicator` interface. See
...
//!
//! SIMD entry points:
//! - `dpo_simd_by_assets`: compute DPO for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `dpo_simd_by_options`: compute DPO for one asset with N different
...
use tulip_rs::indicators::dpo::{Dpo, IndicatorState as DpoState, INPUTS, OPTIONS};
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn dpo_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut DpoStateHandle));
    }
}
...
unsafe fn dpo_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
...
unsafe fn dpo_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
...
mod tests {
    ...
    fn test_dpo_indicator() {
        unsafe {
            ...
            let real = vec![10.0, 11.0, 12.0, 13.0, 14.0, 15.0];

            let inputs_array = [real.as_ptr()];
    ...
    fn test_dpo_batch() {
        unsafe {
            ...
            dpo_state_free(state);
        }
    }

    #[test]
    fn test_dpo_simd_by_assets() {
        unsafe {
            // Two assets, each with real prices
            let real1 = vec![10.0, 11.0, 12.0, 13.0];
            let real2 = vec![20.0, 21.0, 22.0, 23.0];

            // Asset inputs: each asset has INPUTS(1) pointer
            let assets_inputs1 = [real1.as_ptr()];
            let assets_inputs2 = [real2.as_ptr()];
            ...
            let result =
                dpo_simd_by_assets(inputs_ptr, 2, real1.len(), options_ptr, std::ptr::null(), 0);
    ...
    #[test]
    fn test_dpo_simd_by_options() {
        unsafe {
            // Single asset with real prices (need enough data for period=5)
            let real = vec![10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0];
            ...
            let result =
                dpo_simd_by_options(inputs_ptr, real.len(), options_ptr, 2, std::ptr::null(), 0);
