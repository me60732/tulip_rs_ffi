//! extern "C" wrapper for `nvi`, mirroring the core `tulip_rs` crate's
//! `Nvi::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::nvi::Nvi::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_nvi`'s
...
//!
//! SIMD entry points:
//! - `nvi_simd_by_assets`: compute NVI for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//!
//! NOTE: `nvi` has OPTIONS=0, so there is no `nvi_simd_by_options` function.

use std::os::raw::c_void;

use tulip_rs::indicator_types::{Indicator, TIndicatorState};
use tulip_rs::indicators::nvi::{IndicatorState as NviState, Nvi, INPUTS};
...
/// Opaque state handle returned by `nvi_indicator()` and consumed by
/// `nvi_batch()` / `nvi_state_free()`.
pub type NviStateHandle = NviState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn nvi_info() -> CIndicatorInfo {
    pack_info(&Nvi::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn nvi_min_data(_options: *const f64) -> usize {
    Nvi::min_data(&[])
}
...
#[no_mangle]
pub unsafe extern "C" fn nvi_indicator(
    ...
    match Nvi::indicator(&inputs, &[], optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn nvi_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut NviStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn nvi_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut NviStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn nvi_simd_by_assets(
    ...
    match num_assets {
        2 => nvi_simd_by_assets_n::<2>(inputs, data_len, optional_outputs, numoptional),
        4 => nvi_simd_by_assets_n::<4>(inputs, data_len, optional_outputs, numoptional),
        8 => nvi_simd_by_assets_n::<8>(inputs, data_len, optional_outputs, numoptional),
        16 => nvi_simd_by_assets_n::<16>(inputs, data_len, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn nvi_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    numoptional: usize,
    ...
    match Nvi::indicator_by_assets::<N>(&refs, &[], optional) {
        Ok((results, states)) => {
...
mod tests {
    ...
    #[test]
    fn test_nvi_min_data() {
        unsafe {
            assert!(nvi_min_data(std::ptr::null()) > 0);
        }
    }
    ...
    fn test_nvi_batch() {
        ...
        unsafe {
            ...
            let batch_result =
                nvi_batch(state, more_inputs.as_ptr(), data_len, std::ptr::null(), 0);
    ...
    fn test_nvi_simd_by_assets() {
        ...
        unsafe {
            ...
            for i in 0..NUM_ASSETS {
                nvi_state_free(*result.states.add(i));
            }
