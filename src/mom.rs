//! extern "C" wrapper for `mom`, mirroring the core `tulip_rs` crate's
//! `Mom::indicator` / `IndicatorState::batch_indicator` interface with a
...
//! order as `tulip_rs::indicators::mom::Mom::INFO.inputs`), `options`
//! is a flat array of `OPTIONS` values -- exactly like `ti_mom`'s
...
//!
//! SIMD entry points:
//! - `mom_simd_by_assets`: compute MOM for N assets simultaneously,
//!   sharing a single options array. N must be 2, 4, 8, or 16.
//! - `mom_simd_by_options`: compute MOM for one asset with N different
...
use tulip_rs::indicators::mom::{IndicatorState as MomState, Mom, INPUTS, OPTIONS};
...
/// Opaque state handle returned by `mom_indicator()` and consumed by
/// `mom_batch()` / `mom_state_free()`.
pub type MomStateHandle = MomState;
...
/// don't free them.
#[no_mangle]
pub extern "C" fn mom_info() -> CIndicatorInfo {
    pack_info(&Mom::INFO)
}
...
#[no_mangle]
pub unsafe extern "C" fn mom_min_data(options: *const f64) -> usize {
    let options: [f64; OPTIONS] = *(options as *const [f64; OPTIONS]);
    Mom::min_data(&options)
}
...
#[no_mangle]
pub unsafe extern "C" fn mom_indicator(
    ...
    match Mom::indicator(&inputs, &options, optional) {
        Ok((rows, state)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn mom_batch(
    state: *mut c_void,
    inputs: *const *const f64,
    data_len: usize,
    optional_outputs: *const bool,
    ...
    let state = &mut *(state as *mut MomStateHandle);
...
/// after this call.
#[no_mangle]
pub unsafe extern "C" fn mom_state_free(state: *mut c_void) {
    if !state.is_null() {
        drop(Box::from_raw(state as *mut MomStateHandle));
    }
}
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn mom_simd_by_assets(
    ...
    match num_assets {
        2 => mom_simd_by_assets_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => mom_simd_by_assets_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => mom_simd_by_assets_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => mom_simd_by_assets_n::<16>(inputs, data_len, options, optional_outputs, numoptional),
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn mom_simd_by_assets_n<const N: usize>(
    inputs: *const *const *const f64,
    data_len: usize,
    options: *const f64,
    optional_outputs: *const bool,
    ...
    match Mom::indicator_by_assets::<N>(&refs, &options, optional) {
        Ok((results, states)) => {
...
///   `bool`s.
#[no_mangle]
pub unsafe extern "C" fn mom_simd_by_options(
    inputs: *const *const f64,
    data_len: usize,
    ...
    match num_option_sets {
        2 => mom_simd_by_options_n::<2>(inputs, data_len, options, optional_outputs, numoptional),
        4 => mom_simd_by_options_n::<4>(inputs, data_len, options, optional_outputs, numoptional),
        8 => mom_simd_by_options_n::<8>(inputs, data_len, options, optional_outputs, numoptional),
        16 => {
            mom_simd_by_options_n::<16>(inputs, data_len, options, optional_outputs, numoptional)
        }
        _ => CSimdResult::err(IndicatorError::InvalidInputs),
    }
}

unsafe fn mom_simd_by_options_n<const N: usize>(
    inputs: *const *const f64,
    data_len: usize,
    options: *const *const f64,
    optional_outputs: *const bool,
    ...
    match Mom::indicator_by_options::<N>(&inputs, &options, optional) {
        Ok((results, states)) => {
            let (outputs, output_lens, num_outputs, num_results) = pack_simd_outputs(results, optional);
            let states = pack_states(states);
            CSimdResult {
...
mod tests {
    ...
    #[test]
    fn test_mom_min_data() {
        let options: [f64; OPTIONS] = [14.0];
        unsafe {
            assert!(mom_min_data(options.as_ptr()) > 0);
        }
    }
    ...
    fn test_mom_batch() {
        ...
        unsafe {
            ...
            let batch_result =
                mom_batch(state, more_inputs.as_ptr(), data_len, std::ptr::null(), 0);
    ...
    fn test_mom_simd_by_assets() {
        ...
        unsafe {
            ...
            for i in 0..NUM_ASSETS {
                mom_state_free(*result.states.add(i));
            }
    ...
    fn test_mom_simd_by_options() {
        ...
        unsafe {
            ...
            let result = mom_simd_by_options(
                inputs.as_ptr(),
                data_len,
                options,
                NUM_OPTION_SETS,
